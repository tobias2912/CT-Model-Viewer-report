...
%problem
Visualizing three-dimensional data as 2D images limits how the data is interpreted and interacted with. 3D printing solves some of this, but has drawbacks related to the printing process, such as printing time and the need for support structures.

With Virtual Reality (VR) technology, the data can be visualized in 3D using a real-life scale to perceive depth and distances better while manipulating data in three dimensions. The improved visualization can improve the preparation phase by increasing the planning capabilities or removing the need for 3D printing.
...
The final VR application is tested by surgeons to evaluate how it affects the surgery planning and CT image interpretation.
...
The reception is varying, some are positive, while some professionals do not want another tool that achieves a similar result. Especially interacting with fractures and prosthetics in three-dimensional space can simplify the planning process.
...
Other helpful people at Helse Vest are Håkon Garfors and Thomas Larsen who have been very helpful in facilitating the project with relevant connections and help me getting the project started. Grete Oline Hole has provided very useful insight on the background of the project and relevant studies, as well as writing guidance.
...
VR has multiple potential benefits, and the surgery planning process can benefit from this. As the users are already looking at 3D models on 2D screens, visualizing in VR could improve the surgeon's capabilities in planning and by this ensure patient safety.
...
This thesis has used qualitative methods by interviewing related personnel to evaluate the planning improvements, including anatomical understanding, cooperation, and the effectiveness of the planning process. Students or inexperienced personnel has been interviewed with regards to learning outcomes.
The functionality of the VR viewer developed as part of this thesis was compared the existing tools used: standard CT images, 3D rendering and 3D printing.
...
\subsection{Visualization of medical imaging}
...
Figure \ref{mat} shows the application Materialise used at Haukeland University Hospital. Both CT slices from different angles and a rendered model is shown side by side.
...
\section{DICOM data visualization}
...
\subsection{Alternative slice visualization}
...
The survey was answered by 4 orthopedic surgeons, 3 professors in radiology or radiography and 2 radiography students. The total participant count of 9 is high enough to give a good indication of program usability~\cite{experience_why_nodate}, being measured to above 75 \% accuracy in a specific study on SUS accuracy~\cite{tullis_comparison_nodate}.

Having only four surgeons means it is difficult to conclude the research questions on surgery preparation fully. However, the semi-structured interview does provide considerable insight into the research question and is discussed later.
...
Surgeons are a busy group to work with, and this has affected testing to some degree. It is difficult to find surgeons with time to spare, and the testing duration has been 15-20 minutes per surgeon because of busy schedules.
...
Before the test, the intended use cases were described to the participants. The intended use case was improving the understanding compared to traditional slice visualization given a complex fracture.
...
\begin{figure}[h!]
  ...
  \small
  visualization changes on second test. The fracture part is split into several pieces, colors are less saturated along with feature improvements.
\end{figure}
...
The goal was to evaluate how the final application could be used as a planning tool, and get information on any other use cases.


The structure in this test was similar to the previous with interview and SUS testing.

\subsubsection{Results}
The surgeons were positive on the idea of using implants in VR: Inserting and manipulating screws and implants in VR is easier than with mouse and keyboard. The surgeons frequently do prosthesis planning with 3D tools, and the three-dimensional movement is cumbersome on a computer. 
Both say it has potential as a planning tool. One said he would use it, given some minor improvements.

One of the larger drawbacks to visualizing fracture pieces is the required model segmentation that is often done manually. This makes the application a more cumbersome solution than standard CT images. A suggestion is including the segmentation in the VR application to reduce the amount of steps and make segmentation easier. In this mode the user would "paint" different parts of the fracture to assign it to each segment.

Reducing the amount of steps necessary is important in all aspects in order to make the application usable. Preferably the model is exported directly from a tool such as Materialize and imported to VR without any additional actions.

On the idea of using VR as an educational tool the surgeons were very positive. A benefit would be learning to use implants, what sizes and length would fit and how it would be placed. For the tool to be a very useful simulation tool, it would have to include proper simulation features such as realistic tools, drills and also haptic feedback on tools.
...
One participant said VR made him nauseas after a while, but some experience would probably make him better.

Some miscellaneous improvements were mentioned during testing. As revealed in previous testing, the model should be able to rotate on all axis. A suggestion was the joysticks could be used for both rotation and adjusting distance to user. Objects should be able to be grouped together, so grabbing will affect all objects in a group.
...
The benefit from VR mainly comes from three-dimensional controller manipulation, and was a clear opinion from all participants. The suggested use of this is fracture interaction, moving implants and measurements.

Several participants said when comparing to 3D printing that he rarely requested prints, of the same reasons mentioned earlier: When viewing CT images he very often recognizes a familiar fracture.
...
%ease of use
The application had two design goals which is being easy to use and facilitate for planning and CT interpretation.
...
The functionality goal is met to a varying degree. The planned features was implemented with good results, while testing revealed some more improvements that could be addressed. How much value the functionality gives is covered in the next section.

There are some user experience errors apparent in testing that should be improved for a better user experience. The following points are identified as the most critical improvements:
...
Measurement worked fine for the users that understood the usage, which gave good feedback on being able to accurately measure distance on several axis. The interface is differs itself from the rest of application because it uses button clicks and could have been reworked to a grab interaction.
...
%SUS begrunne
The SUS scores gathered total 9 answers, divided between the fields of orthopedics and radiography.
...
Controllers are especially beneficial for planning of complex surgeries because of the easy three-dimensional manipulation.
Two out of four surgeons expressed that implant manipulation and planning in VR adds value and can improve planning. VR is likely useful, but it may depend on personal preference and types of surgery, as the surgeons who preferred it frequently worked with prosthetics.
...
The participant count of 5 is not very high, but gives some insight on usability. With a lowest score of 72, it is clear education was a more reasonable target group based on SUS results. The good score also reinforces the positive feedback in interviews.
...
The other goal of giving users a good understanding of the fracture is partly met, VR gives some benefits to visualization, but it is a small increase compared to existing tools.
...
With experienced surgeons as the target group, there was some positive feedback on VR improving the quality of surgery planning. The improvement primarily comes from controller manipulation of objects such as implants in planning of prosthetics.
...
The feedback is varying based on preferences, age and experience and is also affected by a smaller sample size.
...
As this project was evaluated as a educational tool with good results, it could be developed further. The application does not contain any tools directed specifically towards teaching. This use case probably has some potential not fulfilled by this application.

Possible additions are assignments built in to the application, teacher supervision, gamification elements or basic anatomy guides.
...
\begin{figure}[h!]
    \centering
  \makebox[\textwidth][c]{\includegraphics[width=1.4\textwidth]{images/sus.png}}%
	\hfill
...
\begin{figure}[h!]
    \centering
  \makebox[\textwidth][c]{\includegraphics[width=1.4\textwidth]{images/intervju1.png}}%
	\hfill
...
\begin{figure}[h!]
    \centering
  \makebox[\textwidth][c]{\includegraphics[width=1.4\textwidth]{images/intervju.png}}%
	\hfill
	\caption{Interview notes from evaluation}
  \small
\end{figure}
...
\begin{figure}[h!]
    \centering
  \makebox[\textwidth][c]{\includegraphics[width=1.3\textwidth]{images/intervju2.png}}%
	\hfill
	\caption{Interview notes from evaluation}
  \small
\end{figure}
