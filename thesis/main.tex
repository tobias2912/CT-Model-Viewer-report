...
If the data is represented as a 3D mesh, several formats can be used. The Wavefront Object file format (.obj)\cite{obj} is a standard format for objects defined by polygons and surfaces. 3D mesh formats such as .obj differs from DICOM by not storing volume data. This means that any density data is deleted when converting from DICOM.
...
The architecture of the Unity application is based around a singleton 'GameController' module. The GameController manages important states and contains references to the other modules. The modules have different responsibilities like multiplayer or menu interaction and communicates through the GameController. This solution is easy to extend and prevents a lot of cross-references between components.
...
The modules are visualized in figure~\ref{structure}.
