...
With Virtual Reality (VR) technology, the data can be visualized in 3D using a real-life scale to perceive depth and distances better. The improved visualisation can improve the preparation phase by increasing the anatomical understanding or removing the need for 3D printing.

%method
TODO: mer om metode?
A VR application for viewing CT scans was created with assistance from orthopedic surgeons at Haukeland University Hospital and domain experts in radiology. A surgeon has tested the application, suggested features and given feedback throughout the development.
The final VR application is tested by users to evaluate how it affects the surgery planning and CT image interpretation.
...
The reception is varying, some are positive, and some professionals do not want a tool that achieves a similar result to their existing solutions.
Based on the feedback from testing at the hospital, the application is also tested as an educational tool to target less experienced personnel. Radiography students have participated in user tests to explore this use case. The results indicates VR could be a great tool for teaching CT image interpretation to students.
...
VR has multiple potential benefits, and the surgery planning process can benefit from this. As the users are already looking at 3D models on 2D screens, visualizing in VR could improve the surgeon's understanding of the fracture and by this ensure patient safety.
...
%what is different
The application developed in this project differs itself by visualising the model in separate pieces and allows users to adjust the model.
...
Radiology is the use of medical imaging to diagnose and treat disease. A radiologist is a doctor interpreting images, while the technician performs the imaging are radiographers~\cite{swiontkowski_manual_2013}.
...
For evaluating the surgery planning use case, the application was tested on three orthopedic surgeons currently working at Haukeland University Hospital.
%experience
