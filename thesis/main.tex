...
TODO: skrive om ortoped arbeidsprosess
...
Several other variants of VR viewers already exists. Common functionality for most solutions are viewing DICOM files as models, and a basic VR interface for interacting. 
The solutions differ in visualizing the model as a solid mesh or as a volume. ImmersiveView VR \cite{immersive} and MedicalImagingVR\cite{} renders the scan as a transparent volume. Most solutions also includes a standard 2D plane rendering for viewing slices.
Some solutions like medical holodeck\cite{medical_holodeck_medicalholodeck_nodate} offer a detailed visualization with anatomical layers like muscle and bone, but is created manually with a custom dataset and can not be used for viewing a specific fracture.
Some solutions are also created specifically for other areas than orthopedics like Sentiar CommandEP for heart surgery.
Early AR solutions exist intended for use during surgery, including surgical theater\cite{} intended for neurosurgery\cite{thejns}.
Other generic solutions are...

%what is different
This solution differs itself by vizualising the model split up into separate pieces and allowing users to adjust the model in order to plan for internal fixation. 
