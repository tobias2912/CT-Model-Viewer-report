...
Medical imaging is used to locate the fracture or the number of fractures, and tell the surgeon information about anatomy such as fracture line and type of fracture. It can also show bone condition, if any joints are involved and swelling of soft tissue. All this helps comfirm the diagnosis, study the fracture and plan the treatment\cite{ebnezar_textbook_2016}.
%types of medical imaging
The different imaging types have different advantages, a CT scan is better for smaller hairline fractures and skull, pelvis, spine fractures. MRI can aditionally detect soft tissue and ligament injuries. XR is an older but simpler and cheaper technique\cite{ebnezar_textbook_2016}.

\begin{figure}[h!]
    \centering
	\subfloat[][]{\includegraphics[height=120pt]{images/arbeidsflyt.png}}
	\hfill
...
Fracture treatment in orthopedics can be divided into several methods outlined in Textbook of Orthopedics\cite{ebnezar}.
Nonoperative methods include straps, slings or casts. No treatment also suffices in some cases.
These methods have no infection risk or surgical risk.
With operative methods the fracture needs to be fixed internally by implants.
Implants includes screws, nails and plates and are made from steel, titanium or plastics.
Operative is useful for anatomic reduction or is multiple fractures are present.
Treatment of fractures by external fixation is also possible. A external metallic frame is connected to the bone with pins.
...
\begin{figure}[h!]
    \centering
	\subfloat[][]{\includegraphics[height=120pt]{images/agile.png}}
	\hfill
...
One issue is that the user might want to move either the entire model or move a part of the model. Including both could introduce mode issues\cite{experience_modes_nodate}, so the grab interaction always picks up a model part. A joystick input moves the entire model.
